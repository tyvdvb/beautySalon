...
export const Gallery = () => {
	...
	const handlePhotoChange = (changeRatio) => {
		if (changeRatio > 0) {
			setCurrentPhoto((commentIndex) =>
				commentIndex === photos.length - 1 ? 0 : commentIndex + 1
			);
		}
		if (changeRatio < 0) {
			setCurrentPhoto((commentIndex) =>
				commentIndex === 0 ? photos.length - 1 : commentIndex - 1
			);
		}
	};
	return (
		<div className="salonPictures">
			<Button type="text" onClick={() => handlePhotoChange(-1)}>
